...
const Button = ({ children, variant = 'primary', className = '', ...props }) => {
  const base = 'inline-flex items-center justify-center font-medium transition duration-200 rounded-xl px-5 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-offset-1';

  const variants = {
    primary: 'bg-blue-400 text-white hover:bg-blue-700 focus:ring-blue-500 shadow-md hover:shadow-lg',
    outline: 'bg-white text-blue-600 border border-blue-600 hover:bg-blue-50 focus:ring-blue-500 shadow-sm',
...
const Card = ({ children, className = '' }) => (
  <div className={`rounded-2xl border border-gray-200 bg-white/60 backdrop-blur-md shadow-xl overflow-hidden ${className}`}>
    {children}
  </div>
);

const CardHeader = ({ children, icon: Icon }) => (
  <div className="flex items-center justify-between px-6 py-4 bg-gradient-to-r from-pink-500 to-red-100 border-b border-gray-200">
...
const CardFooter = ({ children }) => (
  <div className="px-6 py-4 border-t text-gray-700 text-sm">{children}</div>
);
